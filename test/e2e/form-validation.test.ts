...
import Page from './page/form-page-object';
...
const setupTest = (testFn: { (pageObjects: PageObjects): Promise<void> }) => {
  return useBrowser(async browser => {
    await browser.url('/form-validation.html');
    const page = new Page(browser);
    const formTemplatePage = new FormTemplatePage(browser);

    await page.waitForPageLoaded();
    await testFn({ page, formTemplatePage });
  });
};
